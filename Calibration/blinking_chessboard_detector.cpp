...
void BlinkingChessBoardDetector::on_blinking_frame(timestamp ts, cv::Mat &cb_frame) {
    ...
    auto &keypoints = output_calib_results_.keypoints_; // initialisation de la variable

  

    cv::swap(frame_, new_frame_);
    if (!cv::findChessboardCorners(frame_, grid_size_, keypoints, calibration_flags)) // find point 
        return;
    
    cv::TermCriteria criteria(cv::TermCriteria::EPS + cv::TermCriteria::COUNT, 40, 0.001);
    cv::cornerSubPix(frame_, keypoints, cv::Size(11, 11), cv::Size(-1, -1), criteria); // affinage de la detection des coins
  
    if (!Metavision::are_points_on_grid_radial_distortion(keypoints, cols_, rows_, cos_max_angle_))  // correction de la distorsion s'il y en a
        return;
