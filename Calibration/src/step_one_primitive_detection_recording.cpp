/**********************************************************************************************************************
* This function enables the acquisition and detection of key points for the calibration                               *
* of an event-based fisheye (omnidirectional) camera.                                                                 *
* Inputs:                                                                                                             *
* - The desired number of poses specified by the user                                                                 *
* - The characteristics of the calibration grid (number of rows and columns)                                          *
* - The camera's serial number                                                                                        *
*                                                                                                                     *
* Please read the instructions displayed when you run the code to save the data                                       *
* This code was inspired by Prophesee code, whose camera we used, and is cited in our article :                       *
"A New Stereo Fisheye Event Camera for Fast Drone Detection and Tracking - Daniel Rodrigues Da Costa, Maxime Robic "  ∗  
**********************************************************************************************************************/

//using a part of Metavision Calibration SDK.
...
///
/// It outputs the 2D detections and the 3D pattern in a text file at the end of the pipeline.
...
public:
        ...
        set_consuming_callback([this](const boost::any &data) {
            try {
                ...
                if (input_calib_results) {
                    ...
                    // std::cout << input_calib_results->keypoints_ << '\n'; 
                    //std::cout << '\n' << typeid(input_calib_results->keypoints_).name() << '\n'; 
    ...
    ~DetectionsLoggerStage() {
        if (pts_2d_.empty()) {
            ...
            MV_LOG_WARNING() << "Failed to dump 2D observations to file.";
    }
    ...
    void load_dumped_points() {
        ...
        if (previous_img_size != img_size_) {
            std::stringstream ss;
            ss << "Invalid image size to load. " << previous_img_size << " instead of " << img_size_ << ".";
        ...
        if (previous_pattern.n_cols_ != pattern_3d_.n_cols_ || previous_pattern.n_rows_ != pattern_3d_.n_rows_ ||
            ...
            std::stringstream ss;
            ss << "Invalid pattern geometry to load. Must be "
        ...
        if (previous_pts_2d.end() !=
            ...
            std::stringstream ss;
            ss << "Invalid 2D observations to load. Must be vectors of size " << pattern_3d_.n_pts_ << ".";
            throw std::invalid_argument(ss.str());
        }
...
public:
    PatternFrameGeneratorStage(int width, int height, int cols, int rows, const std::string &output_images_dir_path, const std::string& output_images_name,
        ...
        set_consuming_callback([&](const boost::any &data)  { // [=](bool flagg, bool excepty)
            try {
                auto ts_calib_results = boost::any_cast<CalibResultsData>(data);
                ...
                if (export_frames_) {
                        ...
                        if (flagg) { // warning flagg
                            std::stringstream ss;
                        //ss << base_frames_path_ << frame_id_++ << ".png";
                            ss << output_images_dir_path << "data_img/" << "Im_" << output_images_name << ".png";
                            cv::imwrite(ss.str(), input_calib_results->frame_);


                            std::ofstream outputFile(output_images_dir_path + "data_point/" + output_images_name + ".txt", std::ios::app);

                            pts_.emplace_back(input_calib_results->keypoints_);
                            if (outputFile.is_open()) {
                                for (int i = 0; i < pts_[pts_.size() - 1].size(); ++i) {
                                    outputFile << pts_[pts_.size() - 1][i].x << " " << pts_[pts_.size() - 1][i].y << std::endl;
                                }

                                outputFile << 1 << std::endl;
                                outputFile.close();
                                std::cout << "Points saved to point.txt" << std::endl;
                                flagg = false;
                                pointSaved = true;
                            }
                            else {
...
private:
    bool excepty;
    std::vector<std::vector<cv::Point2f>> pts_;
    bool export_frames_;
...
bool get_pipeline_configuration(int argc, char *argv[], Config &config) {
    const std::string short_program_desc("Tool showing how to use Metavision Calibration SDK to collect 2D detections "
                                         "of a rigid 3D pattern (blinking chessboard) for the "
                                         "calibration of prophesee camera.\n");
    ...
    base_options.add_options()
        ...
        ("number-pose,nbp",       po::value<int>(&config.nbpose_)->default_value(1),  "N� of the pose.")
        ...
        ("output-dir",            po::value<std::string>(&config.output_dir_path_)->default_value("../../../"), "Path to the folder where the 2D detections will be saved. If the folder does not exists, it will be created.")
        ("output-name,o",         po::value<std::string>(&config.output_name_)->default_value("point_t"), "Path to the folder where the 2D detections will be saved. If the folder does not exists, it will be created.")
        ("serial,s",              po::value<std::string>(&config.serial_), "Serial ID of the camera.")
        ...
        ("disable-images-export", po::bool_switch(&config.disable_images_export_)->default_value(true), "Whether or not we should export the images of the detected patterns.")
    ...
    grid_geometry.add_options()
        ...
        ("fisheye", po::bool_switch(&config.use_fisheye_)->default_value(false), "Allows the detection of strongly distorted grids, as when using a fisheye lens. bisousbisou")
        ;
    // clang-format on
...
int main(int argc, char *argv[]) {
    ...
    if (conf_.serial_.empty()){
        ...
        camera = Metavision::Camera::from_serial(conf_.serial_);
    }
    ...
    // Pipeline
    //
    //  0 (Camera) --->--- 1 (filter) ------------>------------ 2 (Pattern detector :
    //                     |                                   |   blinking chessboard or blincking dot)
    //                     v                                   v  
    //                     |                                   |
    //                     v                                   |------------>-----------|
    //                     |                                   |                        |
    ...
    //                     |                                   |                          image .png format + points .txt format)
    //                     v                                   v
    //                     |---->--  6 (Frame Composer) ---<---|
    //                               |
    //                               v
    ...
    auto& pattern_frame_stage = p.add_stage(
        std::make_unique<PatternFrameGeneratorStage>(width, height, conf_.cols_, conf_.rows_, conf_.output_dir_path_,
                                                     conf_.output_name_, conf_.overlay_convex_hull_),
                pattern_detection_stage);
    ...
    MV_LOG_INFO() << "IF YOU ARE READY TO RECORD FOR THE NEXT POSE DETECTIONS PLEASE PRESSE THE r KEY." << std::endl;// << corresponding : flagg 

    disp_stage.set_key_callback([&](Metavision::UIKeyEvent key, int scancode, Metavision::UIAction action, int mods) {
        if (action == Metavision::UIAction::RELEASE) {
            if (key == Metavision::UIKeyEvent::KEY_ESCAPE || key == Metavision::UIKeyEvent::KEY_Q || pointSaved == true) {
                p.cancel();
            }
            if (key == Metavision::UIKeyEvent::KEY_R) {
                flagg = true;
                MV_LOG_INFO() << "At the next detection the points will be recorded " << std::endl;// <<  corresponding : flagg;
            }
        }
    });
    ...
    MV_LOG_INFO() << "Now run step_two_sort to match pixel coordinates with 3D world coordinates."
                  << std::endl;
